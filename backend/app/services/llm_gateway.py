...
from app.models.test_output import GeneratedTest, TestGenerationOutput
...
class LLMGateway:
    ...
    async def call_structured(
        ...
        *,
        ...
        """
        Structured-call wrapper used by service-layer steps.

        ``step_name`` is used by deterministic mock-mode branching.
        """
        if self._use_mock():
            raw = self._mock_json_response(user_prompt, response_model, step_name=step_name)
            est = max(1, len(system_prompt) // 4 + len(user_prompt) // 4 + len(raw) // 4)
    ...
    def _mock_json_response(self, user_prompt: str, schema: type[T], step_name: str | None = None) -> str:
        """Deterministic stub output for every schema the pipeline emits."""
        if step_name == "testgen":
            return self._mock_test_generation(user_prompt).model_dump_json()
        ...
        if schema is TestGenerationOutput:
            return self._mock_test_generation(user_prompt).model_dump_json()
    ...
    def _mock_test_generation(self, user_prompt: str) -> TestGenerationOutput:
        first_file, _ = self._first_file_from_diff(user_prompt)
        basename = (first_file or "file").split("/")[-1]
        return TestGenerationOutput(
            tests=[
                GeneratedTest(
                    comment_title="Mock test for review finding",
                    file_path=first_file or "test_file.py",
                    test_file_path=f"tests/test_{basename}",
                    framework="pytest",
                    test_code=(
                        "import pytest\n\n"
                        "def test_mock_finding():\n"
                        "    # Generated by Sentinel\n"
                        "    assert True  # Replace with actual assertion\n"
                    ),
                    test_name="test_mock_finding",
                    description="Deterministic mock test from the test generator.",
                    category="bug",
                    setup_notes="No extra setup required in mock mode.",
                    confidence=0.3,
                )
            ],
